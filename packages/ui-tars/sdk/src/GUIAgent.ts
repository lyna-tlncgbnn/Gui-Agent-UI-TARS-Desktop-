  ...
  async run(
    ...
    try {
      ...
      while (true) {
        ...
        const dataPreparationTime = Date.now() - dataPreparationStart;

        const modelInvokeStart = Date.now();
        logger.info(`[GUIAgent] Starting model invoke with ${vlmParams.images?.length || 0} images`);
        const {
          prediction,
          parsedPredictions,
          costTime,
        ...
        const networkOverhead = modelInvokeTime - (costTime || 0);
        ...
        const actionExecuteTime = Date.now() - actionExecuteStart;

        const loopTotalTime = Date.now() - loopStart;
        logger.info(
          `[GUIAgent] Loop ${loopCnt} timing breakdown:\n` +
          `  - Screenshot: ${screenshotTime}ms\n` +
          `  - Image Processing: ${imageProcessTime}ms\n` +
          `  - Data Preparation: ${dataPreparationTime}ms\n` +
          `  - Model Invoke (total): ${modelInvokeTime}ms\n` +
          `    - Model Inference (vLLM): ${costTime}ms\n` +
          `    - Network + Overhead: ${modelInvokeTime - (costTime || 0)}ms\n` +
          `  - Action Execution: ${actionExecuteTime}ms\n` +
          `  - Loop Total: ${loopTotalTime}ms`
        );
